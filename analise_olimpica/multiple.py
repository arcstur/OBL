...
def analyse_multiple_exams(main_df, school_dict_object, analyse_year=True):
    ...
    main_df = main_df.sort_values(
        [col["Ano"], col["Olimpíada"]], ascending=[False, True]
    )
    ...
    for year in main_df[col["Ano"]].unique():
        ...
        series = (
            pd.DataFrame({"FID": year_fractal_id_sets})["FID"]
            .value_counts()
            .value_counts()
        )

        year_dict = utils.amount_of_per_participant_qnt(
            {},
            series,
            range_span=[i + 1 for i in range(max_number_oly + 1)],
            quantity_name="alunos",
            property_name_tuple=("inscrição", "inscrições"),
        )
    ...
    for oly in main_df[col["Olimpíada"]].unique():
        ...
        oly_dict = utils.amount_of_per_participant_qnt(
            {},
            series,
            range_span=[i + 1 for i in range(max_number_year + 1)],
            quantity_name="alunos",
            property_name_tuple=("ano de inscrição", "anos de inscrição"),
        )
